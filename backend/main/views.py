...
class MealRecommendationView(APIView):
    ...
    def get(self, request):
        ...
        if total_natrium < 600:
            serial = MealRecommendSerializer({'condition': "저나트륨", 'message': f"어제 섭취하신 나트륨은 {total_natrium}mg입니다. 하루 권장 섭취 나트륨은 600mg으로, 섭취 나트륨이 부족한 것으로 보입니다. 부족한 나트륨 양과 비슷한 음식은 아래와 같습니다. 참고하셔서 끼니를 거르지 마시고, 조금 더 영양가 있는 식사를 해보시는게 어떨까요?", 'data' : []})
