    ...
    def get(self, request):
        ...
        if extra > 0:
            ...
            if total_meal_calorie < recommend_calorie:
                message = f"{find_user.fullname}님의 하루 권장 섭취칼로리는 {recommend_calorie}입니다. 섭취 칼로리가 하루 권장 섭취칼로리보다 적은 것으로 보입니다. \n 끼니를 거르지 말고 영양가 있는 식사를 해보는게 어떨까요?"
            else:
                message = f"{find_user.fullname}님의 하루 권장 섭취칼로리는 {recommend_calorie}입니다. 하루 권장 섭취칼로리는 충족하셨네요! \n 운동하신 만큼 단백질을 보충하는건 어떨까요?"
